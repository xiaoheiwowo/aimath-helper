...
class ImageGradingMarker:
    ...
    def mark_image_with_grading_results(
        ...
        # 估算题目位置
        question_positions = self.estimate_question_positions(image.shape, practice_data)
        
        # 创建批改结果映射
        grading_map = {result.get("question_id"): result for result in grading_results}
        
        # 为每道题添加标记
        for pos in question_positions:
            question_id = pos["question_id"]
            grading_result = grading_map.get(question_id)
        ...
        for i, image_path in enumerate(image_paths):
            try:
                ...
                output_path = self.mark_image_with_grading_results(
                    image_path, grading_results, practice_data
                )
